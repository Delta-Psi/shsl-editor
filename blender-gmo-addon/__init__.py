...
def import_gmo(context, filepath, global_matrix):
    ...
    import_file(collection, os.path.dirname(filepath), chunk_header, chunk_data, global_matrix)
...
def import_file(collection, directory, header, data, global_matrix):
    index = 0
    ...
    while len(data) > 0:
        ...
        chunk_header = data[8:header_size]
        chunk_data = data[header_size:chunk_size]
        import_subfile(subfile_collection, directory, chunk_header, chunk_data, global_matrix)

        data = data[chunk_size:]
        index += 1

def import_subfile(collection, directory, header, data, global_matrix):
    ...
    for index, (chunk_header, chunk_data) in enumerate(textures):
        textures[index] = import_texture(collection, directory, index, chunk_header, chunk_data)
...
def import_texture(collection, directory, index, header, data):
    ...
    stand_pos = texture_path.find(b'stand_')
    if stand_pos != -1:
        texture_path = os.path.join(directory, '..', '..', 'textures', texture_path[stand_pos+6:].decode('ascii'))
